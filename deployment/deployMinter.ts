...
task(`deploy-minter`, `Deploys the Minter contract`)
	...
	.addParam(`lpRewardsContract`, `The LP Rewards Contract address`)
	...
	.setAction(
		async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
			...
			try {
				...
				console.log(
					`📝 Try to verify it manually with: npx hardhat verify --network ${hre.network.name} ${minter.address} ${jellyToken} ${lpRewardsContract} ${stakingRewardsContract} ${owner} ${pendingOwner}`
				);
			}
		}
	);