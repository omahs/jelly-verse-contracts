...
export async function deployAllocator() {
  ...
  const [owner, otherAccount, vesting, vestingJelly] = await ethers.getSigners();

  // Deploy DUSD
  const ERC20TokenFactory: ERC20Token__factory =
    await ethers.getContractFactory("ERC20Token");

  const dusdToken: ERC20Token = await ERC20TokenFactory.deploy(
    "DUSD",
    "dusd"
  );
  ...
  const AllocatorFactory: Allocator__factory =
    await ethers.getContractFactory("Allocator")
